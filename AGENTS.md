...
- 是否阻断由 sub2api 读取该字段并按自己的阈值判断；Adapter 的 `result_block_threshold` 需要手动与 sub2api 保持一致，用于链路测试、兼容 `flagged` 返回值和运行概览统计，不会反向修改 sub2api。
...
- 长纯文本达到 `segment_audit.threshold_chars` 后进入增量分段审核；优先按消息边界、段落和自然句边界切分，重复片段按策略指纹复用决策缓存，只调用模型审核新增或变化片段。
- 分段结果不能直接取最高分阻断。片段达到复核分数、调用失败或模型判定风险时，必须携带相邻片段和最新末段执行上下文复核；复核结果才是最终分数。
- 片段缓存只保存哈希和模型结论，不保存正常内容明文；模型、系统提示词、策略或密钥变化会改变策略指纹并自动失效旧缓存。
- 联网搜索默认关闭。
- 上游异常默认 fail-open，生产环境必须观察错误率和 P95。

关键词匹配类型：

- `contains`（页面显示“中文/短语包含”）：对归一化文本做不区分大小写的子串匹配，并兼容常见空格、标点差异；中文和固定短语优先使用。
- `word_boundary`（页面显示“英文完整词”）：要求关键词前后不是字母、数字或下划线；适合 `sqli`、`rce`、`keygen` 等英文术语，不适合连续中文。
- `regex`（页面显示“高级正则”）：对归一化文本执行不区分大小写的 Go 正则，仅用于确实需要复杂格式匹配的高级配置。
- 关键词只负责决定是否调用模型，不直接决定阻断；默认词组以高召回为目标，由模型结合目标归属和上下文给最终分数。
- 默认保留 `逆向`、`破解`、`绕过` 等高价值宽词；裸 `token`、`cookie`、`shell`、`payload` 改为带窃取或滥用语境的短语，普通 `未成年`、`毒品`、`安眠药` 和旧自伤组不再作为默认触发词。
- 中文短语与英文术语必须拆分分组，分别使用 `contains` 和 `word_boundary`。后台“载入推荐关键词”只替换关键词草稿，不修改模型、密钥、提示词或其它策略，保存后才生效。
- 推荐关键词当前包含 7 个分组，其中英文露骨色情和成人深伪独立成组；`裸照`已经覆盖未成年人裸照语境，不重复维护更窄的同义短语。
- 关键词效果按分组统计命中、已送审和阻断，每个请求在同一分组最多计一次，阻断率为阻断数除以已送审数。命中缓存和直接送审模式不会重复执行关键词初筛，因此不计入分组命中。
- 关键词统计先写入内存，后台每分钟批量持久化到 SQLite 的 `keyword_stats` 表，正常关闭时强制刷盘；统计过程不保存正常请求内容。
...
- 图片不参与文本分段，始终保持完整图文上下文。视觉请求在共享系统提示词之外追加固定的媒介专用审核指令，用于强调实际可见内容、年龄不确定性、医学/游戏/截图边界；不维护第二套独立系统提示词，避免政策漂移。
...
- 事件页只持久化阻断、故障放行、部分故障经上下文复核恢复和模型禁用放行；正常放行和缓存放行只进入运行指标，不保存逐条事件。
- 阻断事件保存归一化文本明文；故障放行和模型禁用放行保留内容指纹，用于关联重复异常，不保存完整明文。
- 上游失败诊断保存调用阶段、片段编号、错误类型、HTTP 状态、上游错误码、上游请求 ID、是否可重试和脱敏摘要。需要区分超时、网络、鉴权、限流、额度不足、上游不可用、内容拒审、空响应和非法 JSON。
- 事件页使用服务端分页。
...
- 联网搜索：文本和图片模型均默认关闭。
- 决策缓存：默认开启。
- 长内容分段：默认开启；4000 字符开始分段，目标片段 1800 字符，重叠 200 字符，最多 8 段，并发 3 路，0.50 分触发上下文复核，复核上下文最多 6000 字符。
...
- `POST /admin/api/keyword-stats/clear`
